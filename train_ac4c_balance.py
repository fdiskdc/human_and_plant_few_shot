"""
AC4C Balanced Dataset Training Script

This script implements:
1. AC4C balanced dataset training (full train set for training, full test set for testing)
...
def main(config_path='json/ac4c_balance.json', checkpoint_path=None):
    ...
    # Setup logging
    logger = setup_logging(Config.log_dir, Config.experiment_name + '_ac4c_balanced')

    # Setup tensorboard
    tb_writer = setup_tensorboard(Config.log_dir, Config.experiment_name + '_ac4c_balanced')

    # Log basic info
    logger.info(f"\n{'='*60}")
    logger.info("AC4C Balanced Dataset Multi-label Classification Training")
    logger.info(f"{'='*60}")
    ...
    # Data directory for balanced AC4C
    ac4c_data_dir = 'npy/ac4c_processed/balanced_ac4c'

    # Load train dataset
    logger.info(f"\nLoading AC4C balanced TRAIN dataset from {ac4c_data_dir}...")
    ...
    # Load test dataset
    logger.info(f"\nLoading AC4C balanced TEST dataset from {ac4c_data_dir}...")
    ...
    for epoch in range(start_epoch, Config.num_epochs + 1):
        ...
        if should_eval:
            ...
            # Print results
            logger.info(f"\n{'='*60}")
            logger.info(f"Epoch {epoch} - AC4C Balanced Dataset Test Results")
            logger.info(f"{'='*60}")
            ...
            log_metrics_to_tensorboard(tb_writer, metrics_unbalance, 'test_ac4c_balanced_unbalance', epoch, key_prefix='group_ac4c_')
            log_metrics_to_tensorboard(tb_writer, metrics_balanceb, 'test_ac4c_balanced_balanceb', epoch, key_prefix='group_ac4c_')

            # Run Few-Shot Benchmark
            logger.info(f"\n>>> Running AC4C Balanced Few-Shot Benchmark <<<")
            # Use empty datasets for unbalanced (not used for balanced training)
            from dataset.ac4c import AC4CDataset
            ...
            run_few_shot_benchmark_ac4c(
                model=model,
                ac4c_balanced_train=train_dataset,
                ac4c_balanced_test=test_dataset,
                ac4c_unbalanced_train=empty_ac4c_train,  # Not used
                ac4c_unbalanced_test=empty_ac4c_test,    # Not used
        ...
        if Config.save_every_epoch:
            epoch_checkpoint_path = os.path.join(Config.checkpoint_dir, f'ac4c_balanced_epoch_{epoch:03d}.pt')
        ...
        if should_eval and current_macro_f1 > best_macro_f1:
            ...
            checkpoint_path = os.path.join(Config.checkpoint_dir, 'ac4c_balanced_best_model.pt')
    ...
    logger.info(f"Best model saved to: {os.path.join(Config.checkpoint_dir, 'ac4c_balanced_best_model.pt')}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train model on AC4C balanced dataset')
    parser.add_argument('--config', type=str, default='json/ac4c_balance.json',
                       help='Path to configuration file (default: json/ac4c_balance.json)')
