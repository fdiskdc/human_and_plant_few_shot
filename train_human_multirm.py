"""
train_human_multirm.py - MultIRM架构人类训练脚本 / MultIRM Architecture Human Training Script

使用 MultIRM 基线 (model_v3: BiLSTM + Bahdanau Attention) 在人类 12 类数据集上训练。
与 train_human.py 共享训练框架,但模型替换为 51nt 滑动窗口的 BiLSTM 注意力架构。
Trains the MultIRM baseline (model_v3: BiLSTM + Bahdanau Attention) on the human 12-class dataset.
Shares the training framework with train_human.py but uses a 51nt sliding window BiLSTM attention model.
...
- 完整指标 (F1, AUC, MCC 等) / Comprehensive metrics
...
- model_v3: 51nt 窗口的 BiLSTM + Bahdanau / 51nt window BiLSTM + Bahdanau
- 命令行参数 / CLI: --config, --gpu, --seed

输出 / Outputs:
- checkpoints/best_multirm.pt: 最佳 MultIRM 模型 / Best MultIRM model
- logs/multirm_*/train_*.log: 训练日志 / Training logs
- logs/multirm_*/results.json: 评估结果 / Evaluation results
- TensorBoard events: 可视化 / Visualization

数据流 / Data Flow:
1. 加载配置 / Load config
2. 构建 dataset.human.Mer100Dataset / Build Mer100Dataset
3. 划分 + 初始化 model_v3 / Split + init model_v3
4. 训练循环 (51nt 窗口) / Training loop (51nt window)
5. 评估 + 保存 / Evaluate and save

相关文件 / Related Files:
- 调用 / Calls: model.multirm.model_v3, dataset.human.Mer100Dataset, utils.{common,metrics,logging}
- 被调用 / Called by: shell scripts, manual CLI invocations

使用示例 / Usage Example:
    python train_human_multirm.py --config json/human.json --gpu 0
...
def main(config_path='json/human.json'):
    ...
    # Initialize RNA classification model with BiLSTM + Bahdanau Attention
    # 初始化RNA分类模型，包含BiLSTM和Bahdanau注意力机制
    logger.info(f"\nCreating model...")
    model = model_v3(
        num_task=Config.num_classes,
        use_embedding=False,  # model_v3 baseline does not use embedding
        use_hierarchical=Config.use_hierarchical,
        # Additional parameters for compatibility (not used by model_v3)
...
if __name__ == "__main__":
    main(config_path='json/human_multirm.json')
