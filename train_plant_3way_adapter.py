"""
Plant 3-Way Multi-class Classification Training Script with "Freeze Head, Train Adapter" Strategy

This script implements "Plant Class 5 vs. Class 8 vs. Class 9" multi-class classification
with a novel training strategy that preserves the pre-trained class priors.

STRATEGY: "Freeze Head, Train Adapter"
- FREEZE: The final output layers (output_proj) which contain class priors
- UNFREEZE: Only the adapter layers (group_mlps, cross_attention) that adapt features

The key insight: The pre-trained model (0-shot) has excellent class priors (knowing Class 9
is dominant). Few-shot training with balanced (1:1:1) support sets would destroy these priors.
By freezing the output layers and only training the adapter, we preserve the decision boundary
while learning to map new features to it.
...
def train_few_shot_3way(model, support_data_list, target_classes, group_indices,
                        config, device, logger, k_shot):
    """
    Train a single model for 3-WAY multi-class classification of Plant classes [5, 8, 9].

    STRATEGY: "Freeze Head, Train Adapter"
    - FREEZE: The final output layers (output_proj) which contain class priors
    - UNFREEZE: Only the adapter layers (group_mlps, cross_attention)

    The key insight: The pre-trained model has excellent class priors (knowing Class 9
    is dominant). Few-shot training with balanced (1:1:1) support sets would destroy these
    priors. By freezing the output layers, we preserve the decision boundary while only
    training the adapter to map new features to it.
    ...
    # ========================================================================
    #
    # The pre-trained model (0-shot) has excellent class priors (knowing Class 9
    # is dominant). Few-shot training uses a balanced (1:1:1) support set, which
    # would force the model to unlearn these priors and destroy the decision
    # boundary, leading to poor generalization on the imbalanced test set.
    #
    # Solution: We need to PRESERVE the decision boundary (the final classification
    # layer) and ONLY train the feature adaptation layers (the "Adapter").
    #
    # FREEZE (contains class priors, must NOT change):
    #   - output_proj, output_proj_4class, output_proj_12class (final classifiers)
    #   - ESPECIALLY the bias of these layers (contains class frequency prior)
    #
    # UNFREEZE (the "Adapter" - adapts features to existing boundary):
    #   - group_mlps (in HierarchicalClassQueryHeadPooling)
    #   - cross_attention (in ClassQueryHead)
    #   - input_proj, query_proj, mlp, dense, att (intermediate layers)
    #
    # ========================================================================

    # Step 1: Freeze everything first
    logger.info("=" * 80)
    logger.info("STRATEGY: Freeze Head, Train Adapter")
    logger.info("=" * 80)
    for param in model.parameters():
        param.requires_grad = False

    # Step 2: FREEZE the final output layers (contains class priors)
    # These layers determine "which class is more probable" and we must trust
    # the pre-trained weights here to preserve the class imbalance prior.
    logger.info("Step 1: FREEZING final output layers (preserving class priors)")
    frozen_output_params = []
    for name, param in model.class_query_head.named_parameters():
        # Keywords that indicate FINAL CLASSIFICATION layers
        output_keywords = ['output_proj', 'classifier', 'final']
        if any(keyword in name.lower() for keyword in output_keywords):
            param.requires_grad = False
            frozen_output_params.append(name)

    logger.info(f"  Frozen output layers: {frozen_output_params}")

    # Step 3: UNFREEZE the adapter layers (feature adaptation, NOT decision boundary)
    # These act as the "Adapter" to align new features to the old boundary.
    logger.info("Step 2: UNFREEZING adapter layers (feature adaptation only)")
    unfrozen_adapter_params = []
    for name, param in model.class_query_head.named_parameters():
        # Keywords that indicate ADAPTER layers (input/intermediate)
        adapter_keywords = [
            'group_mlps',           # Hierarchical: derivation MLPs
            'cross_attention',      # ClassQueryHead: cross-attention
            'input_proj',           # Input projection
            'query_proj',           # Query projection
            'att',                  # Attention layers
            'dense',                # Dense/MLP layers (but not output)
            'mlp',                  # MLP layers (but not output)
            'norm'                  # Normalization layers
        ]

        # Only unfreeze if it's NOT an output layer
        is_output_layer = any(keyword in name.lower() for keyword in ['output_proj', 'classifier', 'final'])
        is_adapter = any(keyword in name.lower() for keyword in adapter_keywords)

        if is_adapter and not is_output_layer:
            param.requires_grad = True
            unfrozen_adapter_params.append(name)

    logger.info(f"  Unfrozen adapter layers: {unfrozen_adapter_params}")

    # Step 4: Log trainable parameters for verification
    trainable_params = list(filter(lambda p: p.requires_grad, model.parameters()))
    num_trainable = sum(p.numel() for p in trainable_params)

    # Log all trainable parameter names in class_query_head for verification
    trainable_head_params = [n for n, p in model.class_query_head.named_parameters() if p.requires_grad]
    logger.info("=" * 80)
    logger.info(f"Trainable parameters in class_query_head: {trainable_head_params}")
    logger.info(f"Total trainable parameters: {num_trainable:,}")
    logger.info("=" * 80)

    # Step 5: Hyperparameters
    # Since we are training a projection layer and not the sensitive output layer,
    # we can afford a slightly higher LR than 1e-6, but still keep it conservative.
    ft_lr = 1e-4

    # Dynamic epochs: Don't overfit on small support sets
    if k_shot <= 10:
        ft_epochs = 30
    else:
        ft_epochs = 50
    ...
    # CROSS-ENTROPY LOSS for multi-class classification with label smoothing
    # Label smoothing prevents over-confidence on small support sets
    ...
    logger.info(f"Training with ft_lr={ft_lr}, ft_epochs={ft_epochs}, aug_factor={aug_factor}")
    logger.info(f"Support set size: {len(support_data_list)} samples")
    ...
    try:
        for epoch in range(ft_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                # === KEY: Label Mapping ===
                # batch.y is (N, 12). We need to convert it to (N,) with values 0,1,2
                ...
                if config.use_hierarchical:
                    ...
                    loss = criterion(logits_class, target_labels)
                else:
            ...
            if batches_processed > 0 and (epoch + 1) % 10 == 0:
                ...
                logger.info(f"3-Way Shot {k_shot} Ep {epoch+1}/{ft_epochs}: CrossEntropy Loss {avg_loss:.4f}")
    finally:
        # Ensure thread cleanup
        prefetcher.cleanup()

    # ========================================================================
    # WEIGHT INTERPOLATION: Prevent Catastrophic Forgetting
    # ========================================================================
    # Even though we freeze the head, interpolating the Adapter weights is still
    # a good safety net.
    # Formula: final_weight = alpha * original_weight + (1 - alpha) * trained_weight
    alpha = 0.5  # Preserve 50% of pre-trained knowledge

    trained_head_state = model.class_query_head.state_dict()
    interpolated_state = {}

    for key in trained_head_state:
        if key in original_head_state:
            # Perform interpolation: final = alpha * original + (1 - alpha) * trained
            interpolated_state[key] = (
                alpha * original_head_state[key] +
                (1 - alpha) * trained_head_state[key]
            )
        else:
            # New parameter (shouldn't happen in class_query_head, but handle gracefully)
            interpolated_state[key] = trained_head_state[key]

    # Load interpolated weights back into the model
    model.class_query_head.load_state_dict(interpolated_state)
    logger.info(f"Weight interpolation applied (alpha={alpha}): Pre-trained and fine-tuned adapter weights merged")
...
def main(config_path='json/plant_single.json', checkpoint_path=None):
    ...
    logger = setup_logging(Config.log_dir, Config.experiment_name + '_3way_adapter')
    tb_writer = setup_tensorboard(Config.log_dir, Config.experiment_name + '_3way_adapter')

    logger.info(f"\n{'='*60}")
    logger.info("Plant 3-Way Multi-class Classification Training")
    logger.info("STRATEGY: Freeze Head, Train Adapter")
    logger.info("         Freeze output layers (preserve class priors)")
    logger.info("         Train only adapter layers (adapt features)")
    logger.info("         Target: Classes 5 (Y), 8 (m5C), 9 (m6A)")
    logger.info(f"{'='*60}")
    ...
    logger.info(f"  Strategy: Freeze Head, Train Adapter | CrossEntropyLoss")
    logger.info(f"{'='*80}")
    logger.info(f"{'='*80}\n")
    ...
    for k_shot in shot_counts:
        logger.info(f"\n--- 3-Way Classification: Running {k_shot}-Shot Learning ---")
        ...
        if k_shot == 0:
            ...
            # Train with 3-WAY multi-class classification
            model = train_few_shot_3way(
                model, support_data_list, target_classes, group_indices,
                Config, Config.device, logger, k_shot
            )
            logger.info(f"Completed {k_shot}-shot 3-way training")
        ...
        # Log results
        logger.info(f"\n{'='*60}")
        logger.info(f"3-Way Classification - {k_shot}-Shot Results")
        logger.info(f"{'='*60}")
        for cls_id in [5, 8, 9]:
        ...
        if tb_writer:
            for cls_id in [5, 8, 9]:
                cls_metrics = metrics[cls_id]
                tb_writer.add_scalar(f'3way/class_{cls_id}_F1', cls_metrics['F1'], k_shot)
                tb_writer.add_scalar(f'3way/class_{cls_id}_Prec', cls_metrics['Prec'], k_shot)
                tb_writer.add_scalar(f'3way/class_{cls_id}_Rec', cls_metrics['Rec'], k_shot)
                tb_writer.add_scalar(f'3way/class_{cls_id}_Acc', cls_metrics['Acc'], k_shot)
                tb_writer.add_scalar(f'3way/class_{cls_id}_AUC', cls_metrics['AUC'], k_shot)
                tb_writer.add_scalar(f'3way/class_{cls_id}_AUPRC', cls_metrics['AUPRC'], k_shot)
    ...
    # =========================================================================
    results_path = os.path.join(Config.checkpoint_dir, 'plant_3way_adapter_fewshot_results.json')
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"3-Way classification results saved to: {results_path}")
    logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Plant 3-way multi-class classification model with Freeze Head, Train Adapter strategy")
    parser.add_argument('--config', type=str, default='json/plant_single.json', help='Path to config file')
    parser.add_argument('--checkpoint', type=str, required=False, help='Path to pre-trained checkpoint',
