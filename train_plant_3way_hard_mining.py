"""
Plant 3-Way Classification - SPLIT TRAINING STRATEGY with Hard Negative Mining

This script implements a SPLIT training strategy to optimize different classes
with different approaches based on their difficulty:

Core Strategy:
--------------
1. Class 8 (m5C): Standard Random Sampling (ALREADY WORKS WELL)
   - Uses the existing `sample_binary_support_set` with random sampling
   - Standard training epochs (30-50 based on shot count)
   - Standard learning rate (1e-4)

2. Class 5 (Y) & Class 9 (m6A): Hard Negative Mining (NEEDS IMPROVEMENT)
   - Uses new `sample_hard_support_set` with intelligent hard negative selection
   - Increased epochs (70) to handle harder samples
   - Lower learning rate (5e-5) for stability with hard samples
   - 0-shot model used to identify confusing negatives

Key Innovations:
--------------
1. SPLIT STRATEGY: Different training approaches for different class difficulties
2. HARD NEGATIVE MINING: Actively selects the most confusing negative samples
3. 0-SHOT GUIDED SAMPLING: Uses initial model predictions to find hard examples
4. INCREASED TRAINING: More epochs and slower LR for difficult classes
5. HYBRID BIAS FREEZING: Retained for ALL classes (preserves priors at low shots)
...
def sample_binary_support_set(dataset, plant_train_indices, k_shot, target_class):
    """
    Sample support set for binary classification (1:1 balanced) using STANDARD RANDOM SAMPLING.
...
def sample_hard_support_set(dataset, plant_train_indices, k_shot, target_class, model, device, config, pool_factor=10):
    """
    Sample support set for binary classification using HARD NEGATIVE MINING.

    Key Innovation:
    --------------
    Instead of random sampling, this function uses the 0-shot model to identify
    the HARDEST negatives (samples that the model currently confuses with the target class).

    Steps:
    1. Sample K positives from target class (random)
    2. Sample K * pool_factor candidate negatives from other classes
    3. Use 0-shot model to predict probabilities on the negative pool
    4. Select K negatives with HIGHEST probability for target class (most confusing)

    Rationale:
    ----------
    By training on hard negatives, the model learns better decision boundaries
    and improves AUC/Precision for difficult classes (Y, m6A).
    ...
    Args:
        ...
        k_shot: Number of shots per class (total samples = 2*k_shot)
        target_class: Target class ID (5, 8, or 9)
        model: The 0-shot RNA model (used for scoring negatives)
        device: Device to run model on
        config: Configuration object
        pool_factor: Factor to expand negative pool (default: 10x)

    Returns:
        list: Support indices (balanced 1:1, total 2*k_shot samples with hard negatives)
    """
    if k_shot == 0 or k_shot == '0':
        return []

    # Step 1: Sample K positives from target class (random)
    target_positives = dataset.get_plant_indices_by_class(target_class)
    valid_positives = list(set(target_positives) & set(plant_train_indices))
    ...
    # Step 2: Sample K * pool_factor candidate negatives from other classes
    other_classes = [c for c in TARGET_CLASSES if c != target_class]
    valid_negatives = []
    for other_c in other_classes:
        other_indices = dataset.get_plant_indices_by_class(other_c)
    ...
    # Expand negative pool for hard mining
    pool_size = k_shot * pool_factor
    if len(valid_negatives) >= pool_size:
        candidate_negatives = np.random.choice(valid_negatives, pool_size, replace=False).tolist()
    else:
        # If not enough negatives, use all available with replacement
        candidate_negatives = np.random.choice(valid_negatives, pool_size, replace=True).tolist()
        print(f"Warning: Only {len(valid_negatives)} negative samples available for class {target_class}, using replacement for pool")

    # Step 3: Use 0-shot model to score the negative pool
    model.eval()  # Important: Use eval mode for scoring

    negative_scores = []
    negative_loader = DataLoader(
        Subset(dataset, candidate_negatives),
        batch_size=64,
        shuffle=False,
        num_workers=2
    )
    ...
    with torch.no_grad():
        for batch in negative_loader:
            ...
            negative_scores.extend(prob.cpu().numpy().tolist())

    # Step 4: Select K negatives with HIGHEST probability (most confusing)
    # These are the "hard negatives" that the 0-shot model currently confuses with target class
    negative_scores = np.array(negative_scores)
    top_k_indices = np.argsort(negative_scores)[-k_shot:][::-1]  # Top K most confusing

    selected_neg = [candidate_negatives[i] for i in top_k_indices]

    # Log hard mining statistics
    avg_hard_score = np.mean([negative_scores[i] for i in top_k_indices])
    avg_easy_score = np.mean(negative_scores)

    print(f"\nHard Negative Mining for Class {target_class}:")
    print(f"  Candidate pool size: {len(candidate_negatives)}")
    print(f"  Selected hard negatives: {len(selected_neg)}")
    print(f"  Avg score (hard): {avg_hard_score:.4f}, Avg score (pool): {avg_easy_score:.4f}")
    print(f"  Hardness ratio: {avg_hard_score/avg_easy_score:.2f}x")

    # Combine positives and hard negatives, then shuffle
    support_indices = selected_pos + selected_neg
    np.random.shuffle(support_indices)
...
# ============================================================================
# Binary Training Function (Updated for Split Strategy)
# ============================================================================

def train_binary_model(model, support_data_list, target_class, config, device, logger, k_shot, use_hard_mining=False):
    """
    Train a single binary classification model for One-vs-Rest task.

    Key Features:
    --------------
    1. DYNAMIC Bias Freezing (based on k_shot) - APPLIED TO ALL CLASSES
    2. BINARY Focal Loss
    3. L2-SP Regularization (prevents drift from 0-shot)
    4. SPLIT STRATEGY: Different hyperparameters based on difficulty
    ...
    Args:
        ...
        logger: Logger instance
        k_shot: Number of shots per class (total samples = 2*k_shot)
        use_hard_mining: Whether hard negative mining was used (affects hyperparameters)
    ...
    # ========================================================================
    # STEP 1: Apply HYBRID Strategy (Dynamic Bias Freezing) - FOR ALL CLASSES
    # ========================================================================
    trainable_params = apply_hybrid_strategy(model, k_shot, logger)

    # ========================================================================
    # STEP 2: Training Hyperparameters (SPLIT STRATEGY)
    # ========================================================================
    class_name = PLANT_CLASS_MAPPING[target_class]['class_name']

    if use_hard_mining:
        # Hard Mining Strategy for Classes 5 & 9 (Y, m6A)
        # Lower LR for stability with hard samples, more epochs for convergence
        ft_lr = 5e-5
        ft_epochs = 70
        logger.info(f"\n{'='*80}")
        logger.info(f"HARD MINING STRATEGY - Class {target_class} ({class_name})")
        logger.info(f"  Rationale: Hard negatives require slower, more stable training")
        logger.info(f"  Learning Rate: {ft_lr} (reduced from 1e-4)")
        logger.info(f"  Epochs: {ft_epochs} (increased from 30-50)")
        logger.info(f"{'='*80}\n")
    else:
        # Standard Strategy for Class 8 (m5C)
        # Standard hyperparameters for easy class
        ft_lr = 1e-4
        if k_shot <= 10:
            ft_epochs = 30
        else:
            ft_epochs = 50
        logger.info(f"\n{'='*80}")
        logger.info(f"STANDARD STRATEGY - Class {target_class} ({class_name})")
        logger.info(f"  Rationale: Standard random sampling with proven performance")
    ...
    logger.info(f"  Target Class: {target_class} ({class_name})")
...
# ============================================================================
# Main Training Function (SPLIT STRATEGY)
# ============================================================================
...
def main(config_path='json/plant_single.json', checkpoint_path=None):
    ...
    logger = setup_logging(Config.log_dir, Config.experiment_name + '_3way_hard_mining')
    tb_writer = setup_tensorboard(Config.log_dir, Config.experiment_name + '_3way_hard_mining')

    logger.info(f"\n{'='*80}")
    logger.info("Plant 3-Way Classification - SPLIT TRAINING STRATEGY with Hard Negative Mining")
    logger.info(f"{'='*80}")
    logger.info("Core Strategy:")
    logger.info("  - Class 8 (m5C): Standard Random Sampling (preserves proven performance)")
    logger.info("  - Class 5 (Y) & 9 (m6A): Hard Negative Mining (boosts difficult classes)")
    logger.info("  - Hybrid Bias Freezing: Applied to ALL classes (preserves priors at low shots)")
    ...
    if not checkpoint_path or not os.path.exists(checkpoint_path):
        logger.error("Checkpoint path is required for split training strategy!")
        return
    ...
    # ========================================================================
    # SPLIT TRAINING STRATEGY LOOP
    # ========================================================================
    logger.info(f"\n{'='*80}")
    logger.info(f"{'='*80}")
    logger.info(f"SPLIT TRAINING STRATEGY")
    logger.info(f"  Branch A: Class 8 (m5C) - Standard Random Sampling")
    logger.info(f"  Branch B: Class 5 (Y) & 9 (m6A) - Hard Negative Mining")
    logger.info(f"  Common: Hybrid Bias Freezing (for ALL classes)")
    logger.info(f"{'='*80}")
    logger.info(f"{'='*80}\n")
    ...
    for target_class in TARGET_CLASSES:
        ...
        # Determine strategy based on class difficulty
        if target_class == 8:
            # Branch A: Standard Random Sampling for m5C (already works well)
            logger.info(f"\n{'='*80}")
            logger.info(f"BRANCH A: Class {target_class} (m5C) - STANDARD RANDOM SAMPLING")
            logger.info(f"  Rationale: m5C already achieves high performance with standard training")
            logger.info(f"  Strategy: Preserve proven approach with standard hyperparameters")
            logger.info(f"{'='*80}\n")
            use_hard_mining = False
        else:
            # Branch B: Hard Negative Mining for Y and m6A (need improvement)
            logger.info(f"\n{'='*80}")
            logger.info(f"BRANCH B: Class {target_class} ({class_name}) - HARD NEGATIVE MINING")
            logger.info(f"  Rationale: {class_name} needs improvement - use hard mining to boost AUC/Precision")
            logger.info(f"  Strategy: 0-shot guided hard mining + increased epochs + lower LR")
            logger.info(f"{'='*80}\n")
        ...
        for k_shot in shot_counts:
            ...
            logger.info(f"Strategy: {'HARD MINING' if use_hard_mining else 'STANDARD RANDOM'}")
            logger.info(f"{'='*80}\n")
            ...
            if k_shot == 0:
                ...
                # ====================================================================
                # STEP 2: Sample binary support set (SPLIT STRATEGY)
                # ====================================================================
                np.random.seed(Config.random_seed + k_shot + target_class)

                if use_hard_mining:
                    # Branch B: Hard Negative Mining for Classes 5 & 9
                    logger.info(f"Using Hard Negative Mining for difficult class {target_class}...")
                    support_indices = sample_hard_support_set(
                        full_dataset, plant_train_indices, k_shot, target_class,
                        model, Config.device, Config, pool_factor=10
                    )
                else:
                    # Branch A: Standard Random Sampling for Class 8
                    logger.info(f"Using Standard Random Sampling for proven class {target_class}...")
                ...
                # ====================================================================
                # STEP 3: Train binary model (SPLIT STRATEGY)
                # ====================================================================
                ...
                model = train_binary_model(
                    ...
                    use_hard_mining=use_hard_mining  # Pass strategy flag
                )
            ...
            # Log to TensorBoard
            if tb_writer:
                strategy_suffix = '_hard' if use_hard_mining else '_standard'
                tb_writer.add_scalar(f'binary_{target_class}{strategy_suffix}/F1', metrics['F1'], k_shot)
                tb_writer.add_scalar(f'binary_{target_class}{strategy_suffix}/Prec', metrics['Prec'], k_shot)
                tb_writer.add_scalar(f'binary_{target_class}{strategy_suffix}/Rec', metrics['Rec'], k_shot)
                tb_writer.add_scalar(f'binary_{target_class}{strategy_suffix}/Acc', metrics['Acc'], k_shot)
                tb_writer.add_scalar(f'binary_{target_class}{strategy_suffix}/AUC', metrics['AUC'], k_shot)
                tb_writer.add_scalar(f'binary_{target_class}{strategy_suffix}/AUPRC', metrics['AUPRC'], k_shot)
    ...
    # =========================================================================
    results_path = os.path.join(Config.checkpoint_dir, 'plant_3way_hard_mining_results.json')
    ...
    for target_class in TARGET_CLASSES:
        class_name = PLANT_CLASS_MAPPING[target_class]['class_name']
        strategy = 'hard_mining' if target_class in [5, 9] else 'standard_random'
        class_key = f"class_{target_class}_{class_name}_{strategy}"
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"Split Training Strategy results saved to: {results_path}")
    logger.info(f"  - Class 8 (m5C): Standard Random Sampling")
    logger.info(f"  - Classes 5 (Y) & 9 (m6A): Hard Negative Mining")
    logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Plant 3-way classification using SPLIT TRAINING STRATEGY with Hard Negative Mining"
    )
