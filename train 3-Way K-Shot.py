"""
Plant 3-Way Multi-class Classification Training Script

This script implements "Plant Class 5 vs. Class 8 vs. Class 9" multi-class classification:
1. Train ONE SINGLE model to classify between three Plant classes: 5 (Y), 8 (m5C), 9 (m6A)
2. Use CrossEntropyLoss for multi-class classification (not binary)
3. Sample K shots for EACH of the 3 classes (total samples = 3*K)
4. Label mapping: Class 5 -> 0, Class 8 -> 1, Class 9 -> 2

Key Changes from Binary Approach:
- NO outer loop over classes - train once for all 3 classes
- CrossEntropyLoss instead of BCEWithLogitsLoss
- Label indices mapped from [5, 8, 9] to [0, 1, 2]
- Model pruned to output 3 classes instead of 1
- Evaluation metrics adapted for 3-way classification
...
def train_few_shot_3way(model, support_data_list, target_classes, group_indices,
    ...
    # === STRATEGY: Freeze Backbone, Train Head Only ===
    # 1. Freeze everything first
    for param in model.parameters():
        param.requires_grad = False
        
    # 2. Unfreeze ONLY the Class Query Head
    ...
    logger.info(f"Frozen Backbone strategy active. Trainable params: {num_trainable}")
    
    # 4. Adjust Hyperparameters
    # Reduced LR to prevent catastrophic forgetting - original rate was too aggressive
    ft_lr = 1e-6
    
    # Dynamic epochs: Don't overfit on 2 samples
    if k_shot <= 10:
        ft_epochs = 30
    else:
        ft_epochs = 50
    ...
    # ========================================================================
    # Interpolate between pre-trained weights and fine-tuned weights
    # to preserve valid feature boundaries learned during pre-training.
    ...
    logger.info(f"Weight interpolation applied (alpha={alpha}): Pre-trained and fine-tuned weights merged")
...
def evaluate_3way_detailed(model, test_loader, device, target_classes, use_hierarchical=True):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # Assuming map_labels_3way is used inside loader or here
...
def main(config_path='json/plant_single.json', checkpoint_path=None):
    ...
    logger = setup_logging(Config.log_dir, Config.experiment_name + '_3way_kshot')
    tb_writer = setup_tensorboard(Config.log_dir, Config.experiment_name + '_3way_kshot')
    
    logger.info(f"\n{'='*60}")
    logger.info("Plant 3-Way Multi-class Classification Training")
    logger.info("Strategy: Single model for Classes 5 (Y), 8 (m5C), 9 (m6A)")
    logger.info("         CrossEntropyLoss | K-shot per class")
    logger.info(f"{'='*60}")
    ...
    logger.info(f"  Strategy: Multi-class Classification | CrossEntropyLoss")
    logger.info(f"{'='*80}")
    logger.info(f"{'='*80}\n")
    ...
    # =========================================================================
    results_path = os.path.join(Config.checkpoint_dir, 'plant_3way_fewshot_results.json')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Plant 3-way multi-class classification model (Classes 5, 8, 9)")
