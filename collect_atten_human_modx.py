...
def collect_attention_outputs(model, dataloader, device, accumulate_batches=10):
    ...
    Returns:
        dict: Dictionary with keys:
            - context_vector: [num_samples, hidden_dim]
            - attention_weights: [num_samples, num_classes, seq_len]
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Processing samples")):
            ...
            with autocast():
                ...
                logits, context_vector, attention_weights = model(
...
def main(config_path='json/human.json', checkpoint_path=None, output_path='npy/modx_atten.npz',
    ...
    print(f"\n{'='*60}")
    print("ModX Model Attention Output Collector")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Collect attention outputs for modx model')
    parser.add_argument('--config', type=str, default='json/human.json',
                        help='Path to configuration file')
    parser.add_argument('--checkpoint', type=str, default='logs/modx_rna_classification_20260203_135128/checkpoints/best_model.pt',
                        help='Path to model checkpoint (optional)')
    parser.add_argument('--output', type=str, default='npy/modx_atten.npz',
