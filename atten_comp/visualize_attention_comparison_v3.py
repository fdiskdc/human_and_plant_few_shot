"""
atten_comp/visualize_attention_comparison_v3.py - m6A注意力对比可视化 v3 (共享Y轴上限) / m6A Attention Comparison v3 (Shared Y-axis Cap)

v3 版本:在保留背景色块窗口边界的基础上,每条序列内 4 个模型共享同一个 Y 轴上限。
超过共享上限的注意力峰设为 NaN,不在图中绘制。
V3 version: keeps background window blocks and uses one shared y-axis cap per sequence across all 4 models.
Attention values above the shared cap are set to NaN and are not drawn.
...
使用示例 / Usage Example:
    python atten_comp/visualize_attention_comparison_v3.py
...
def main(mrmodn_path='npy/mrmodn_full_atten.npz',
    ...
    print(f"{'='*60}")
    print("m6A Attention Comparison Visualization v3 (Shared Y-axis Cap)")
    print(f"{'='*60}")
