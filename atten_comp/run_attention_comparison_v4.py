"""
atten_comp/run_attention_comparison_v4.py - 长序列注意力对比实验 v4 / Long-Sequence Attention Comparison v4

v4 版本流水线:1) 选代表性序列 2) mRModN 全长推理 3) MultiRM 51nt 滑动窗口 4) modX 1001nt 全长 (无滑动窗口) 5) EvoRMD 41nt 滑动窗口
6) 生成 v4 对比图 (动态 Top-K Recall)。
V4 pipeline: 1) Select sequences 2) mRModN full 3) MultiRM 51nt 4) modX full 5) EvoRMD 41nt 6) Generate v4 figures with dynamic Top-K Recall.

功能模块 / Modules:
- 选代表性序列 / Select representative sequences
- 4 模型推理 (modX 改为全长) / 4-model inference (modX full-length)
- v4 对比图生成 / V4 comparison figure generation
...
输出 / Outputs:
- figs_atten/xxx/seq_XXXX.pdf, seq_XXXX.png: v4 对比图 / V4 comparison figures
...
4. 生成 v4 对比图 (动态 Top-K Recall) / Generate v2 figures (background blocks)

相关文件 / Related Files:
- 调用 / Calls: select_representative_sequences, inference_*, atten_comp/inference_modx_full, atten_comp/visualize_attention_comparison_v4
- 被调用 / Called by: shell scripts, manual CLI

使用示例 / Usage Example:
    python atten_comp/run_attention_comparison_v4.py
...
#!/usr/bin/env python3
"""
Run All v4: Long-Sequence Attention Comparison Experiment
...
Executes the full pipeline:
  ...
  6. Visualize attention comparison v4 (dynamic Top-K Recall)

Usage:
    python atten_comp/run_attention_comparison_v4.py
    python atten_comp/run_attention_comparison_v4.py --output_dir figs_atten/my_exp --top_n 50
"""
...
def build_scripts(top_n, low_n, output_dir, clip_quantile):
    return [
        ...
        ("atten_comp/visualize_attention_comparison_v4.py", ["--output_dir", output_dir, "--clip_quantile", str(clip_quantile)]),
    ]


def main(output_dir='fig/attention_comparison', top_n=200, low_n=200, clip_quantile=0.99):
    scripts = build_scripts(top_n, low_n, output_dir, clip_quantile)
    total_start = time.time()

    print(f"{'='*60}")
    print("Long-Sequence Attention Comparison Experiment v4")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run full attention comparison pipeline v4")
    ...
    parser.add_argument('--low_n', type=int, default=500,
