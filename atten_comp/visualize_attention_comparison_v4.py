"""
atten_comp/visualize_attention_comparison_v4.py - m6A注意力对比可视化 v4 (动态Top-K Recall) / m6A Attention Comparison v4 (Dynamic Top-K Recall)

v4 版本:保留 v3 的共享 Y 轴上限,并按每条序列的 m6A 位点数量动态选择 Top-K Recall。
规则: >20 个位点用 Top-30, 10-20 个位点用 Top-20, 1-9 个位点用 Top-(2*m6A位点数), 0 个位点显示 N/A。
V4 version: keeps the v3 shared y-axis cap and selects Top-K Recall dynamically by m6A site count.
...
3. 绘制 4 子图 (共享 Y 轴上限) / Draw 4 subplots (shared y-axis cap)
4. 保存 PNG + PDF / Save PNG + PDF

相关文件 / Related Files:
- 调用 / Calls: numpy, matplotlib, sliding_window_utils
- 被调用 / Called by: atten_comp/run_attention_comparison_v3.py

使用示例 / Usage Example:
    python atten_comp/visualize_attention_comparison_v4.py
...
def compute_topk_recall(attn, sites, k=50):
    ...
    K is selected per sequence by choose_topk_by_site_count() in v4.

    Args:
        attn: [1001] attention weights for m6A (L1-normalized)
        sites: [1001] site labels (0 = no modification, M6A_LABEL = m6A)
        k: number of top attention positions to consider
...
def choose_topk_by_site_count(n_true):
    if n_true > 20:
        return 30
    if n_true >= 10:
        return 20
...
def compute_shared_ymax(attns, clip_quantile=DEFAULT_CLIP_QUANTILE):
    values = np.concatenate([np.asarray(attn, dtype=float).ravel() for attn in attns])
    values = values[np.isfinite(values)]
    if values.size == 0:
        return 1.0

    y_max = float(np.quantile(values, clip_quantile))
...
def hide_values_above(attn, y_max):
    attn_plot = np.asarray(attn, dtype=float).copy()
    attn_plot[attn_plot > y_max] = np.nan
    return attn_plot


def visualize_sequence(seq_idx, rank, mrmodn_attn, multirm_attn,
                       modx_attn, evormd_attn, sites_1001, output_dir,
                       clip_quantile=DEFAULT_CLIP_QUANTILE):
    ...
    y_max = compute_shared_ymax(attns, clip_quantile)
    ...
    for ax, attn, (key, label, color, boundaries) in zip(axes, attns, MODEL_META):
        ax.set_facecolor(MORANDI['bg'])

        x = np.arange(len(attn))
        attn_plot = hide_values_above(attn, y_max)

        ax.fill_between(x, attn_plot, alpha=0.20, color=color)
        ax.plot(x, attn_plot, color=color, linewidth=1.8)
        ...
        k = choose_topk_by_site_count(n_true)
        if k > 0:
            topk = compute_topk_recall(attn, sites_1001, k=k)
            recall_text = f'Top-{k} Recall = {topk:.1%}'
        else:
            recall_text = 'Top-K Recall = N/A'
        ax.text(0.99, 0.92, recall_text,
...
def main(mrmodn_path='npy/mrmodn_full_atten.npz',
    ...
    print(f"{'='*60}")
    print("m6A Attention Comparison Visualization v4 (Dynamic Top-K Recall)")
    print(f"{'='*60}")
    ...
    print(f"Shared y-axis cap: per-sequence quantile {clip_quantile:g}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clip_quantile', type=float, default=DEFAULT_CLIP_QUANTILE,
                        help='Per-sequence quantile used as the shared y-axis cap (default: 0.99)')
    args = parser.parse_args()
    main(args.mrmodn, args.multirm, args.modx, args.evormd, args.output_dir, args.clip_quantile)
