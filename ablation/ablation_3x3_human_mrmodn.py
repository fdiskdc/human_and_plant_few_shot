...
相关文件 / Related Files:
- 调用 / Calls: model.mrmodn, dataset.{human,plant,plant_single}, scipy.stats
...
def balanced_binary_split(pos_indices, neg_indices, train_ratio=0.7, seed=RANDOM_SEED):
    ...
    # Sample negative with replacement if needed, print warning
    if n_neg_train > n_neg_total:
        warnings.warn(f"Not enough negative samples: need {n_neg_train}, have {n_neg_total}. Using sampling with replacement.")
        ...
        if n_neg_test > len(neg_indices_remaining):
            warnings.warn(f"Not enough negative samples for test: need {n_neg_test}, have {len(neg_indices_remaining)}. Using sampling with replacement.")
...
def build_plant_binary_loaders(
    ...
    Negative: zero samples (all zeros in all classes) sampled from zero pool

    Zero is pre-split into train/test pools to avoid leakage. We sample matching
    number of negatives from the corresponding pool.
    ...
    if shared_zero_splits is None:
        raise ValueError("shared_zero_splits must be provided for plant binary loaders")

    neg_train = shared_zero_splits["train"][:n_neg_train]
    neg_test = shared_zero_splits["test"][:n_neg_test]

    print(f"  Shared zero pool: train={len(shared_zero_splits['train'])}, test={len(shared_zero_splits['test'])}")
    print(f"  Balanced: train={len(pos_train) + len(neg_train)} ({len(pos_train)} pos, {len(neg_train)} neg), "
          f"test={len(pos_test) + len(neg_test)} ({len(pos_test)} pos, {len(neg_test)} neg)")
...
def evaluate_binary_task(model, test_loader, target_class, device, use_hierarchical):
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if use_hierarchical and isinstance(out, tuple):
                logits_12 = out[0]
            else:
                logits_12 = out if not isinstance(out, tuple) else out[0]

            # Extract the logit for target class
            logit = logits_12[:, target_class]
...
def create_model_for_task(Config, task_class_index):
    """Create a fresh model for the binary task.
    We still use 12-class output architecture for compatibility with pretrained backbone."""
    model = RNA_ClassQuery_Model(
        ...
        num_classes=Config.num_classes,  # Keep 12 classes for architecture compatibility
        ...
        for batch in pbar:
            ...
            if use_amp and torch.cuda.is_available():
                ...
                with torch.amp.autocast('cuda'):
                    ...
                    logit = logits_12[:, task.class_index]
...
def generate_3x3_plots(all_records):
    ...
    for i, human_class in enumerate(task_names):
        for j, plant_class in enumerate(task_names):
            ...
            if i == 2:
                ax.set_xlabel(
                    f"Plant {plant_class}", fontsize=18, fontfamily="Times New Roman"
                )
            if j == 0:
                ax.set_ylabel(
                    f"Human {human_class}", fontsize=18, fontfamily="Times New Roman"
                )
