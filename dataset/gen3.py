...
class Gen3Dataset(Dataset):
    ...
    def __init__(self, mode='train', data_dir='../npy/3gen', cache_dir=None, use_cache=True, preload_cache=True):
        ...
        if cache_dir is None:
            self.CACHE_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'cache', 'gen3')
        else:
        ...
        # 使用3gen目录的数据
        print(f"使用内存映射加载3gen数据: {data_dir}")
        
        # 使用mmap_mode='r'进行内存映射加载，支持多进程共享内存
        self.sequences = np.load(f'{data_dir}/seq.npy', mmap_mode='r')
        self.full_labels = np.load(f'{data_dir}/1001loc.npy', mmap_mode='r')
        self.y_12class = np.load(f'{data_dir}/12loc.npy', mmap_mode='r')
        self.y_4class = np.load(f'{data_dir}/4loc.npy', mmap_mode='r')
        
        print(f"Gen3数据集初始化完成 (mode={mode}):")
    ...
    def _extract_attention_masks_N(self, sequence_bytes):
        ...
        Args:
            sequence_bytes (np.array): |S1类型的字节数组或标量字节字符串，长度为1001
    ...
    def _get_batch_cache_path(self):
        ...
        Returns:
            str: 批量缓存文件完整路径
        """
        return os.path.join(self.CACHE_DIR, f"gen3_{self.mode}_{BATCH_CACHE_FILE}")
    ...
    def _get_cache_key(self, sequence_str, idx):
        ...
        cache_key = f"gen3_{self.mode}_{idx}_{sequence_hash}"
        return cache_key
    ...
    def _one_hot_encode_optimized(self, sequence_bytes):
        ...
        Args:
            sequence_bytes (np.array): |S1类型的字节数组或标量字节字符串
            
        Returns:
            np.array: one-hot编码后的数组，shape为(len(seq), 4)
        """
        # 检查是否为标量字符串（从mmap加载的数据是标量）
        if sequence_bytes.ndim == 0:
            # 标量字节字符串，转换为字节数组
        ...
        if len(bytes_data) != TARGET_LENGTH:
            ...
            if len(bytes_data) < TARGET_LENGTH:
                # 填充'N'
                padding = np.full(TARGET_LENGTH - len(bytes_data), 78, dtype=np.uint8)  # 78是'N'的ASCII码
        ...
        # 使用预建的映射表进行快速查找
        # bytes_data是(1001,)数组，映射表是(256,4)
        # 输出形状为(1001, 4)
...
if __name__ == "__main__":
    ...
    dataset = Gen3Dataset(data_dir='npy/3gen')
