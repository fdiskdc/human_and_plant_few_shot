"""
Plant vs. Zero Binary Classification Training Script

This script implements "Plant vs. Zero (Background)" binary classification:
1. Loop over plant classes [5, 8, 9] - training a specialized BINARY model for each class

NEW APPROACH: Plant vs. Zero (Background)
- Positives: k samples from Plant (specific target class)
- Negatives: k samples from Zero dataset (background/unmodified)
- This is a TRUE binary classification: Plant Modification vs. No Modification

2. Strict 1:1 Balanced Sampling:
   - Target Class: k positives from Plant (specific class)
   - Negative Pool: k samples from Zero dataset (background)
   - Example: Class 5 uses k Plant-5 samples + k Zero background samples

3. Focal Loss (gamma=2.0):
   - Addresses class imbalance by focusing on hard examples
   - More effective than standard BCE for few-shot learning
...
- Zero: zero/zero_seq.npy, zero/zero_label12.npy, zero/zero_label4.npy, zero/zero_label1001.npy
"""
...
from utils.few_shot import prepare_training_batch, sample_support_set_with_zero
...
class FocalLoss(nn.Module):
    ...
    - alpha_t is the weighting factor (default: 0.25)
    - gamma is the focusing parameter (default: 2.0)

    Higher gamma increases the focus on hard-to-classify examples.
    """
...
def filter_evaluation_data_for_binary_classification(y_true, y_prob, y_4class, y_4prob,
    ...
    """
    Filter evaluation data to enforce "Target Plant Class vs. Zero Background" binary classification.

    This function filters out Plant samples that are NOT the target class, ensuring that:
    - Positives: Target Plant Class samples (e.g., Class 5 samples when training Class 5)
    - Negatives: Zero/Background samples ONLY (excludes other Plant classes like 8 and 9)
    ...
    # 1. KEEP: Plant samples that are POSITIVE for the target class (y_true[:, target_class] == 1)
    # 2. KEEP: Zero samples (indices >= num_plant) regardless of their label
    # 3. DISCARD: Plant samples that are NEGATIVE for the target class (other Plant classes)

    valid_indices = []

    for idx, test_idx in enumerate(test_indices):
        # Check if this is a Zero sample (background)
        is_zero_sample = (test_idx >= num_plant)

        if is_zero_sample:
            # Keep all Zero samples as negatives
            valid_indices.append(idx)
        else:
            # This is a Plant sample - only keep if it's positive for target class
            # y_true[idx] is shape [12], check the target_class position
            if y_true[idx, target_class] == 1:
                # This is a positive sample for our target class - KEEP
                valid_indices.append(idx)
            # else: This is a Plant sample from a different class - DISCARD
    ...
    print(f"  [Filter] Filtered test set: {len(filtered_y_true)} samples")
    print(f"  [Filter] Removed {len(y_true) - len(filtered_y_true)} non-target Plant samples")
...
def train_few_shot_single_class_binary(model, support_data_list, target_class, group_idx,
    ...
    batch_list = prepare_training_batch(
        support_data_list, [], aug_factor, k_shot, shuffle=True, logger=None
    )
    ...
    for epoch in range(ft_epochs):
        ...
        if batches_processed > 0 and (epoch + 1) % 10 == 0:
            ...
            logger.info(f"Binary Class {target_class} Shot {k_shot} Ep {epoch+1}/{ft_epochs}: Focal Loss {avg_loss:.4f}")
...
def main(config_path='json/plant.json', checkpoint_path=None):
    ...
    logger = setup_logging(Config.log_dir, Config.experiment_name + '_plant_vs_zero_binary')
    tb_writer = setup_tensorboard(Config.log_dir, Config.experiment_name + '_plant_vs_zero_binary')

    logger.info(f"\n{'='*60}")
    logger.info("Plant vs. Zero Binary Classification Training")
    logger.info("Strategy: Plant (Positives) vs. Zero (Background/Negatives)")
    logger.info("         Strict 1:1 Balanced | Focal Loss | Adaptive Threshold")
    logger.info(f"{'='*60}")
    ...
    # Load Plant+Zero Combined Dataset
    logger.info(f"Loading Plant+Zero Combined Dataset...")
    ...
    # Precompute structures for Zero dataset if needed
    cache_stats = full_dataset.get_cache_stats()
    if not cache_stats['zero_cache'].get('exists', False):
        logger.info("Zero cache not found, precomputing Zero structures...")
        full_dataset.precompute_zero_structures(batch_size=100)

    # Plant target classes to iterate over
    target_classes = [5, 8, 9]

    # Prepare Plant train/test indices for few-shot sampling
    # Use only Plant indices for training (0 to num_plant-1)
    plant_train_indices = []
    plant_test_indices = []
    ...
    # Get Zero train/test split (deterministic, no data leakage)
    zero_train_indices, zero_test_indices = full_dataset.get_zero_split(
        test_ratio=0.2,
        seed=Config.random_seed
    )

    # Combined test set: Plant test + Zero test
    combined_test_indices = plant_test_indices + zero_test_indices

    test_dataset = Subset(full_dataset, combined_test_indices)
    ...
    logger.info(f"Zero Train: {len(zero_train_indices)}, Zero Test: {len(zero_test_indices)}")
    logger.info(f"Total Test Set: {len(combined_test_indices)} (Plant + Zero)")
    ...
    for target_class in target_classes:
        ...
        logger.info(f"  Strategy: Plant (Positives) vs. Zero (Background/Negatives)")
        logger.info(f"  Strict 1:1 Balanced | Focal Loss (gamma=2.0) | Adaptive Threshold")
        logger.info(f"{'='*80}")
        logger.info(f"{'='*80}\n")
        ...
        for k_shot in shot_counts:
            ...
            if k_shot == 0:
                ...
                # BINARY Balanced Sampling: k_shot positives from Plant + k_shot negatives from Zero
                np.random.seed(Config.random_seed + target_class + k_shot)
                support_indices = sample_support_set_with_zero(
                    full_dataset, plant_train_indices, zero_train_indices, k_shot, target_class
                )
                ...
                # Plant samples are < num_plant, Zero samples are >= num_plant
                ...
                logger.info(f"Plant vs. Zero Sampling: {len(support_indices)} samples ({pos_count} Plant pos + {neg_count} Zero neg)")
            ...
            # ====================================================================
            # CRITICAL: Filter evaluation data to enforce "Target Plant vs. Zero" only
            # Remove Plant samples from other classes (e.g., when training Class 5,
            # remove Classes 8 and 9 from the negative pool)
            # ====================================================================
            logger.info(f"[Binary Class {target_class}] Applying Target-vs-Zero filtering to evaluation data...")
            filtered_y_true, filtered_y_prob, filtered_y_4class, filtered_y_4prob, valid_idx = \
                filter_evaluation_data_for_binary_classification(
                    ...
                    combined_test_indices, full_dataset, target_class
                )

            # Run evaluation on FILTERED data (Target Plant + Zero only)
        ...
        # ====================================================================
        # Full Data Training Mode (After Few-shot Experiments)
        # ENFORCED STRATEGY: Target Plant Class (Positives) vs. Zero (Negatives ONLY)
        # ====================================================================
        logger.info(f"\n{'='*80}")
        logger.info(f"Class {target_class}: Starting FULL DATA Training")
        logger.info(f"  STRATEGY: Plant (Positives) vs. Zero (Negatives ONLY)")
        logger.info(f"  - Positives: ALL Plant samples for Class {target_class}")
        logger.info(f"  - Negatives: ZERO samples from zero_train_indices (EXCLUSIVELY)")
        logger.info(f"  - Balance: 1:1 (Num Positives = Num Negatives)")
        logger.info(f"  - Split: 80% Train / 20% Test")
        logger.info(f"{'='*80}")

        # ====================================================================
        # A. Data Preparation for Full Mode
        # ====================================================================
        # POSITIVES: Get ALL positive indices for target class from Plant dataset
        all_pos_indices = full_dataset.get_plant_indices_by_class(target_class)

        # NEGATIVES: EXCLUSIVELY from Zero train split (NO other Plant classes allowed)
        # This ensures true binary classification: Target Plant vs. Background (Zero)
        all_neg_pool = zero_train_indices.copy()
        logger.info(f"[Full Mode] Negative pool: zero_train_indices only ({len(all_neg_pool)} Zero samples)")

        # Downsample negatives to match positives (1:1 balance)
        num_pos = len(all_pos_indices)
        if len(all_neg_pool) >= num_pos:
            selected_neg_indices = np.random.choice(all_neg_pool, num_pos, replace=False).tolist()
        else:
            logger.warning(f"Not enough Zero negatives ({len(all_neg_pool)}) to match Plant positives ({num_pos})")
            selected_neg_indices = all_neg_pool

        # Combine and shuffle
        full_balanced_indices = all_pos_indices + selected_neg_indices
        np.random.shuffle(full_balanced_indices)

        # 80/20 Train-Test Split
        split_idx = int(0.8 * len(full_balanced_indices))
        full_train_indices = full_balanced_indices[:split_idx]
        full_test_indices = full_balanced_indices[split_idx:]

        logger.info(f"Full Mode: Total={len(full_balanced_indices)} (Plant pos={num_pos}, Zero neg={len(selected_neg_indices)})")
        logger.info(f"Full Mode: Train={len(full_train_indices)}, Test={len(full_test_indices)}")

        # Create DataLoaders for Full Mode
        full_train_dataset = Subset(full_dataset, full_train_indices)
        full_test_dataset = Subset(full_dataset, full_test_indices)

        full_train_loader = DataLoader(
            full_train_dataset, batch_size=32, shuffle=True,
            num_workers=8, pin_memory=True
        )
        full_test_loader = DataLoader(
            full_test_dataset, batch_size=32, shuffle=False,
            num_workers=8, pin_memory=True
        )

        # B. Reset Model to Original Pre-trained Weights
        logger.info(f"Resetting model to original pre-trained checkpoint state...")
        model.load_state_dict(original_state_dict,strict=False)

        # Clear any pruning buffers
        if hasattr(model.class_query_head, 'valid_class_indices'):
            del model.class_query_head.valid_class_indices
        if hasattr(model.class_query_head, 'valid_group_indices'):
            del model.class_query_head.valid_group_indices

        # C. Unfreeze All Parameters (Full Fine-tuning)
        logger.info(f"Unfreezing ALL parameters for full fine-tuning...")
        for param in model.parameters():
            param.requires_grad = True

        # D. Standard Training Loop (30 Epochs)
        ...
        logger.info(f"Starting full fine-tuning: 30 epochs, batch_size=32, lr=1e-4")

        for epoch in tqdm(range(30), desc=f"Full Mode Class {target_class}", ncols=100):
            # Training phase
            model.train()
            train_loss = 0.0
            train_batches = 0
            ...
            with torch.no_grad():
                for batch in full_test_loader:
                    ...
                    if Config.use_hierarchical:
                        logits_class, _ = model(batch.x, batch.edge_index, batch.batch)
                        test_logits = logits_class[:, target_class]
                    else:
                        logits_class = model(batch.x, batch.edge_index, batch.batch)
                        test_logits = logits_class[:, target_class]

                    test_probs = torch.sigmoid(test_logits).cpu().numpy()
                    test_labels = batch.y[:, target_class].cpu().numpy()
            ...
            if (epoch + 1) % 5 == 0:
                logger.info(f"Full Mode Ep {epoch+1}/30: Train Loss={avg_train_loss:.4f}, Test AUC={test_auc:.4f}, Test AUPRC={test_auprc:.4f}")
        ...
        logger.info(f"Full Mode Training Complete!")
        logger.info(f"Best Test AUC: {best_test_auc:.4f} at Epoch {best_epoch}")

        # E. Store Full Mode Results
        all_results[target_class]['full'] = {
            'unbalance': {
                f'group_plant_class_{target_class}_auc': best_metrics['test_auc'],
                f'group_plant_class_{target_class}_auprc': best_metrics['test_auprc'],
            },
            'balanceb': {
                f'group_plant_class_{target_class}_auc': best_metrics['test_auc'],
                f'group_plant_class_{target_class}_auprc': best_metrics['test_auprc'],
        ...
        logger.info(f"Class {target_class} Full Mode results stored with key 'full'")

    # =========================================================================
    # Final Results Summary
    # =========================================================================
    logger.info(f"\n{'='*80}")
    logger.info("FINAL PLANT vs. ZERO BINARY CLASSIFICATION RESULTS SUMMARY")
    logger.info("Positive: Plant (specific class) | Negative: Zero (background)")
    logger.info(f"{'='*80}")
    ...
    # =========================================================================
    results_path = os.path.join(Config.checkpoint_dir, 'plant_vs_zero_fewshot_binary_results.json')
    ...
    for target_class, shot_results in all_results.items():
        class_name = PLANT_CLASS_MAPPING[target_class]['class_name']
        serializable_results[f"plant_vs_zero_class_{target_class}_{class_name}"] = {}
        for k_shot, metrics_dict in shot_results.items():
            ...
            serializable_results[f"plant_vs_zero_class_{target_class}_{class_name}"][shot_key] = convert_to_serializable(metrics_dict)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Plant vs. Zero binary classification models (Plant pos, Zero neg, 1:1 balanced)")
