#!/usr/bin/env python3
"""
gen3_spatial_motif.py
Spatial motif analysis for Gen3 RNA modification model.
Combines localized IG motif analysis (from gen3_localized_ig_motif.py) with
latent-space clustering and sequence logo generation (from SpatialMotif_nobackground.py).

双管线架构:
  管线 A (默认): localized IG → STREME/Tomtom 分析 (原 gen3_localized_ig_motif.py 逻辑)
  管线 B (--spatial_motif): GCN嵌入 → PCA+KMeans聚类 → 全局 IG 归因 → 序列标志图

输出目录: ipynb/spatial_motif/output/gen3/
"""
...
# ===========================================================================
# RNA 字母表与 one-hot 编码 (移植自 gen3_localized_ig_motif.py)
# ===========================================================================
...
# DBSCAN eps defaults (Gen3 仅含 m6A)
offset = 16
...
# ===========================================================================
# 管线 A: Localized IG motif 分析 (原 gen3_localized_ig_motif.py 逻辑)
# ===========================================================================
...
def run_pipeline(args):
    ...
    # 中英文：不覆盖 cache_dir，使用 Gen3Dataset 默认的 cache/gen3，匹配已有的 gen3_test_structures_cache.npz
    # English: do not override cache_dir, use Gen3Dataset default cache/gen3 to match existing gen3_test_structures_cache.npz
...
def main():
    p = argparse.ArgumentParser(description='Gen3 spatial motif analysis (dual pipeline)')
    ...
    p.add_argument('--output_dir', default=os.path.join(PROJECT_ROOT, 'ipynb', 'spatial_motif', 'output', 'gen3'))
    p.add_argument('--data_dir', default=None)
    p.add_argument('--mods', default='m6A',
                   help='Gen3 数据集仅含 m6A (Gen3 dataset only contains m6A).')
