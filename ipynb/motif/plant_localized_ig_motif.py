...
多标签处理说明：
1. IG 侧：plant 数据按位点标签提供 RNA 修饰多标签任务，同一条 1001nt 序列可以有多个修饰位点、
   同一个样本也可以同时包含多个修饰类别。本脚本不会把样本强行改成单标签；它直接读取
   PlantDataset.full_labels 中的位点级标签，对用户指定的每一种修饰分别收集
...
1. IG side: the plant dataset provides site-level RNA modification labels. One 1001nt
   ...
   PlantDataset.full_labels directly, collects one (sample_idx, site_pos, class_idx)
...
# ---------------------------------------------------------------------------
# 主流程：加载模型和全量 plant 数据，逐修饰提取 IG motif 并运行 STREME/Tomtom。
# Main pipeline: load model and full plant data, then extract IG motifs per modification
...
def run_pipeline(args):
    ...
    # 中文：复用 dataset/plant.py 的 PlantDataset，确保 one-hot、LinearFold 结构边
    #      和模型输入流程保持一致。
    # English: reuse PlantDataset from dataset/plant.py so one-hot encoding,
    #          LinearFold structure edges, and model inputs match training.
    ...
    # ---- Full-data analysis ----
    # 中文：motif 分析使用全量 plant 数据，不再先构造 train/test split。
    # English: motif analysis uses the full plant dataset; no train/test split is created here.
    ...
    for mod_name, class_idx in target_mods:
        ...
        for ei, (sidx, spos, cidx) in enumerate(mod_events):
            try:
                # 中文：从 PlantDataset 取样本和真实 LinearFold 结构边。
                # English: fetch the sample and real LinearFold structure edges from PlantDataset.
                data = dataset[sidx]
                # 中文：edge_index/标签仍复用 PlantDataset；x 用本脚本的 T/U 等价映射显式重建。
                # English: edge_index/labels still come from PlantDataset; x is rebuilt with this script's T/U-equivalent mapping.
...
def main():
    ...
    p = argparse.ArgumentParser(description='Plant localized IG motif analysis (v2)')
    p.add_argument('--config', default='json/plant.json')
    p.add_argument('--checkpoint', required=True)
    p.add_argument('--output_dir', default=os.path.join(PROJECT_ROOT, 'ipynb', 'motif', 'output', 'localized_ig_plant'))
