...
Localized Integrated Gradients motif analysis for Gen3 12-class RNA modification model.

v2 — Full-data loading via Gen3Dataset,
...
多标签处理说明：
1. IG 侧：Gen3 数据是 12 类修饰多标签任务，同一条 1001nt 序列可以有多个修饰位点、
   同一个样本也可以同时包含多个修饰类别。本脚本不会把样本强行改成单标签；它直接读取
   Gen3Dataset.full_labels 中的位点级标签，对用户指定的每一种修饰分别收集
...
1. IG side: the Gen3 dataset is a 12-class multi-label modification task. One 1001nt
   ...
   Gen3Dataset.full_labels directly, collects one (sample_idx, site_pos, class_idx)
...
Smoke test:
    python ipynb/motif/gen3_localized_ig_motif.py \
      --config json/3gen.json \
      --checkpoint logs/old/rna_classification_20260129_195404/checkpoints/epoch_090.pt \
      --output_dir ipynb/motif/output/debug_localized_ig_gen3 \
...
from dataset.gen3 import Gen3Dataset, MOD_NAMES, LABEL_MAPPING
...
# ---------------------------------------------------------------------------
# 配置加载：读取 json/3gen.json 里的模型、数据和训练配置。
# Config loading: read model, data, and training settings from json/3gen.json.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 序列辅助函数：从 Gen3Dataset 的字节数组恢复原始 RNA 序列字符串。
# Sequence helpers: recover raw RNA strings from Gen3Dataset byte arrays.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 主流程：加载模型和全量 Gen3 数据，逐修饰提取 IG motif 并运行 STREME/Tomtom。
# Main pipeline: load model and full Gen3 data, then extract IG motifs per modification
...
def run_pipeline(args):
    ...
    # 中文：如果有 --trained_checkpoint，直接使用；否则调用 ensure_converged_checkpoint 训练至收敛。
    # English: if --trained_checkpoint is given, use it directly; otherwise train to convergence via ensure_converged_checkpoint.
    ...
    # ---- Dataset via Gen3Dataset ----
    # 中文：复用 dataset/gen3.py 的 Gen3Dataset，确保 one-hot、LinearFold 结构边、
    #      y_site 标签和训练/推理流程保持一致。
    # English: reuse Gen3Dataset from dataset/gen3.py so one-hot encoding,
    #          LinearFold structure edges, y_site labels, and model inputs match training.
    data_dir = args.data_dir or os.path.join(
        PROJECT_ROOT, cfg['data'].get('gen3_data_dir', 'npy/3gen'))
    cache_dir = os.path.join(
        PROJECT_ROOT, cfg['data'].get('gen3_cache_dir',
                                      cfg['data'].get('cache_dir', 'npy/cache')))
    print(f"[Data] loading Gen3Dataset from {data_dir}")
    dataset = Gen3Dataset(
        mode='train',
        data_dir=data_dir,
        cache_dir=cache_dir,
    ...
    # 中文：motif 分析只使用训练集索引，不使用 validation 数据。
    # English: motif analysis uses only train indices; validation data is never used for plotting.
    if train_indices is not None:
        ...
        print(f"[Data] using all gen3 samples: {len(indices)}")
    ...
    for mod_name, class_idx in target_mods:
        ...
        for ei, (sidx, spos, cidx) in enumerate(mod_events):
            try:
                # 中文：从 Gen3Dataset 取样本和真实 LinearFold 结构边。
                # English: fetch the sample and real LinearFold structure edges from Gen3Dataset.
                data = dataset[sidx]
                # 中文：edge_index/标签仍复用 Gen3Dataset；x 用本脚本的 T/U 等价映射显式重建。
                # English: edge_index/labels still come from Gen3Dataset; x is rebuilt with this script's T/U-equivalent mapping.
...
def main():
    ...
    p = argparse.ArgumentParser(description='Gen3 localized IG motif analysis (v2)')
    p.add_argument('--config', default='json/3gen.json')
    p.add_argument('--checkpoint', default=None,
                   help='(Legacy) direct checkpoint path. Use --trained_checkpoint instead.')

    # ---- Training / checkpoint CLI ----
    p.add_argument('--init_checkpoint',
                   default=_DEFAULT_INIT_CKPT,
                   help='Human pre-trained checkpoint for weight initialization')
    p.add_argument('--trained_checkpoint', default=None,
                   help='Path to already-trained checkpoint (skip training). '
                        'Use "None" or omit to train from scratch.')
    p.add_argument('--force_retrain', action='store_true',
                   help='Force retrain even if best_converged.pt exists')
    p.add_argument('--max_train_epochs', type=int, default=None,
                   help='Max training epochs (default: config num_epochs)')
    p.add_argument('--early_stop_patience', type=int, default=10,
                   help='Early stopping patience (default 10)')
    p.add_argument('--early_stop_min_delta', type=float, default=1e-4,
                   help='Min F1 delta for early stopping (default 1e-4)')
    p.add_argument('--val_ratio', type=float, default=0.1,
                   help='Validation split ratio (default 0.1)')
    p.add_argument('--train_batch_size', type=int, default=None,
                   help='Batch size for training (default: config batch_size)')

    p.add_argument('--output_dir', default=os.path.join(PROJECT_ROOT, 'ipynb', 'motif_apply', 'output', 'gen3', 'localized_ig'))
    ...
    # Legacy --checkpoint compatibility: if provided, treat as --trained_checkpoint
