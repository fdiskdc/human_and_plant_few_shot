...
def compute_topk_recall(attn, sites):
    """
    Top-K Recall: fraction of true m6A sites captured by top-K attention positions.

    K = number of true sites. If the model's top-K attention positions
    perfectly cover all true sites, recall = 1.0.

    Args:
        attn: [1001] attention weights for m6A
...
def visualize_sequence(seq_idx, rank, mrmodn_attn, multirm_attn,
    ...
    for ax, (key, label, color, boundaries) in zip(axes, MODEL_META):
        ...
        if boundaries:
            for bw in boundaries:
                ax.axvline(x=bw, color=MORANDI['boundary'], linewidth=1.2,
                           linestyle='--', alpha=0.70, zorder=1)

        topk = compute_topk_recall(attn, sites_1001)
        ax.text(0.99, 0.92, f'Top-{n_true} Recall = {topk:.1%}',
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color=MORANDI['boundary'], linewidth=1.2,
...
def main(mrmodn_path='npy/mrmodn_full_atten.npz',
         ...
         modx_path='npy/modx_segmented_atten.npz',
    ...
    print(f"{'='*60}")
    print("m6A Attention Comparison Visualization")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--modx',    type=str, default='npy/modx_segmented_atten.npz')
    parser.add_argument('--evormd',  type=str, default='npy/evormd_segmented_atten.npz')
    parser.add_argument('--output_dir', type=str, default='fig/attention_comparison')
